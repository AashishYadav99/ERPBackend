...
const store = async (req, res, next) => {
  const {
    ...
    status,
    addedby,
    address = [], // Initialize with empty array if not provided
    banks = [], // Initialize with empty array if not provided
    contacts = [], // Initialize with empty array if not provided
    warehouses = [], // Initialize with empty array if not provided
  } = req.body;

  try {
    let getNumber = await codesettingGet("location");
    await codesettingupdate("location");
    ...
    if (address.length > 0) {
      for (let a = 0; a < address.length; a++) {
        await LocationAddressModel.create({
    ...
    // Handle bank data if available
    if (banks.length > 0) {
      for (let b = 0; b < banks.length; b++) {
        await LocationBankModel.create({
          location_id: location.id,
    ...
    // Handle contact data if available
    if (contacts.length > 0) {
      for (let c = 0; c < contacts.length; c++) {
        await LocationContactModel.create({
          location_id: location.id,
    ...
    // Handle warehouse data if available
    if (warehouses.length > 0) {
      for (let w = 0; w < warehouses.length; w++) {
        await LocationWarehouseModel.create({
          location_id: location.id,
