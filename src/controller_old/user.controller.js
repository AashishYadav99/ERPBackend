...
const login = async (req, res, next) => {
    ...
    try {
        ...
        if (!passwordMatched) {
            res.status(400).json(ResponseFormatter.setResponse(false, 400, 'password not match', 'Error', ''));
            return
        }
        ...
        const refreshToken = jwt.sign({
            email: user.email,
            userId: user.id
        }, "secreatnoteforfestivalapp", { expiresIn: '300d' });

        user = { ...user.toJSON(), ...{ token: jWtToken, refreshToken: refreshToken, type: 'user' } }
        res.status(200).json(ResponseFormatter.setResponse(true, 200, '', '', user));

    } catch (error) {
        // res.status(400).json(ResponseFormatter.setResponse(false, 400, message.lablevalue, 'Error', ''));
        res.status(200).json(ResponseFormatter.setResponse(false, 500, '', '', error));
    }
}
...
const forgotPassword = async (req, res, next) => {
    ...
    try {
        ...
        if (emailCount === 0) {
            ...
            res.status(404).json(ResponseFormatter.setResponse(false, 404, message.lablevalue, 'Error', ''));
            return
        }
        ...
        const emailContent = await emailContentModel.findOne({
            ...
            const placeholders = { username: user[0].first_name, resetpasswordlink: `http://festivalappapi.cph4.ch/reset-password/${token}` }

            var content = res.content.replace('##reset link##', `<a href="http://festivalappapi.cph4.ch/reset-password/${token}">
        ...
        res.status(200).json(ResponseFormatter.setResponse(true, 200, message.lablevalue, 'Success', { token: token }));
    } catch (error) {
        console.log(error);
        let message = await translate_message(13, language_id);
        res.status(400).json(ResponseFormatter.setResponse(false, 400, message.lablevalue, 'Error', error.message));
...
const resetPassword = async (req, res, next) => {
    ...
    try {
        ...
        if (!tokenFound) {
            ...
            res.status(404).json(ResponseFormatter.setResponse(false, 400, message.lablevalue, 'Error', ''))
        }
        ...
        res.status(200).json(ResponseFormatter.setResponse(true, 200, message.lablevalue, 'Success', User))
    } catch (error) {
        let message = await translate_message(32, language_id);
        res.status(400).json(ResponseFormatter.setResponse(false, 400, message.lablevalue, 'Error', error.message))
    }
...
const changePassword = async (req, res, next) => {
    ...
    try {
        ...
        if (!User) {
            res.status(404).json(ResponseFormatter.setResponse(false, 400, 'User not found', 'Error', ''))
        }
        ...
        if (!passwordMatched) {
            res.status(400).json(ResponseFormatter.setResponse(false, 400, 'Old password not match', 'Error', ''));
            return
        }
        ...
        res.status(200).json(ResponseFormatter.setResponse(true, 200, 'Password changed', 'Success', User))
    } catch (error) {
...
const Check_user = async (req, res, next) => {
    ...
    try {
        ...
        res.status(200).json(ResponseFormatter.setResponse(false, 200, message.lablevalue, '', ''));
...
const WebsendOtp = async (req, res, next) => {
    ...
    try {
        ...
        res.status(200).json(ResponseFormatter.setResponse(true, 200, message.lablevalue, 'Success', sendMsgOtp))
    } catch (error) {
        let message = await translate_message(13, language_id);
        res.status(400).json(ResponseFormatter.setResponse(false, 400, message.lablevalue, 'Error', error.message));
    }
...
const getuserById = async (req, res, next) => {
    ...
    try {
        ...
        if (UserCheck === null || UserCheck === '') {
            ...
            res.status(400).json(ResponseFormatter.setResponse(false, 400, message.lablevalue, 'Error', 'Deactive'));
            return
        }
        ...
        if (!UserDetail) {
            ...
            if (UserDetail.photo != '') {
                UserDetail_photo = "https://cph4.ch/uploads/artist_user/" + UserDetail.photo;
            } else {
            ...
            if (UserDetail.art_image != '') {
                UserDetail_art_image = "https://cph4.ch/uploads/artist_user/" + UserDetail.art_image;
            } else {
            ...
            for (var i = 0; i < serviceRes.length; i++) {
                ...
                if (product_list.service_display_id == null) {
                    product_list.service_display_id = '#' + zeroPad(product_list.service_id, 6);
                } else {
                    product_list.service_display_id = '#' + zeroPad(product_list.service_id, 6);
                }
                ...
                if (product_list.file != '') {
                    ...
                    product_list.file = "https://cph4.ch/uploads/service/" + product_list.file;
                } else {
                ...
                servive_detail.available_ticket = tickets_sold + ' OF ' + number_of_tickets;
            ...
            for (var j = 0; j < RelatedUserDetail.length; j++) {
                ...
                if (RelatedUserDetail[j].photo != "") {
                    RelatedUserDetail_photo = "https://cph4.ch/uploads/artist_user/" + RelatedUserDetail[j].photo;
                }
                ...
                if (RelatedUserDetail[j].art_image != "") {
                    RelatedUserDetail_art_image = "https://cph4.ch/uploads/artist_user/" + RelatedUserDetail[j].art_image;
                }
...
const refreshToken = async (req, res, next) => {
    ...
    try {
        ...
        res.status(200).json(ResponseFormatter.setResponse(true, 200, '', '', token));

    } catch (error) {
        let message = await translate_message(13, language_id);
        res.status(400).json(ResponseFormatter.setResponse(false, 400, message.lablevalue, 'Error', 'Session expired'));
    }
}
